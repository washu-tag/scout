...
```mermaid
flowchart LR
    P0["Phase 0<br/>chart publishing"]:::done
    P1["Phase 1<br/>release automation"]:::done
    P2["Phase 2<br/>build pipeline + haul"]:::seeding
    P3["Phase 3<br/>deploy/ base + config artifact"]:::wip
    P4["Phase 4<br/>site repos + dev cutover"]:::todo
    P5["Phase 5<br/>on-prem cutover"]:::todo
    P0 --> P2
    P1 --> P2
    P2 --> P3 --> P4 --> P5
    classDef done fill:#c8e6c9,stroke:#2e7d32,color:#1b5e20
    classDef seeding fill:#fff3cd,stroke:#f9a825,color:#795548
    classDef wip fill:#bbdefb,stroke:#1565c0,color:#0d47a1
    classDef todo fill:#eceff1,stroke:#90a4ae,color:#546e7a
```

- **Phase 0 (done):** charts publish as versioned OCI artifacts; derived config (spark-defaults) is chart-owned.
- **Phase 1 (done, live):** release-please drives releases (#641 merged); the release PR `chore(main): release 4.2.0` is open for a human to merge.
- **Phase 2 (merged; go-live pending):** the per-merge build pipeline, signed haul, and retention are merged (#642, #643, and the upstream-image haul #637). `publish-haul` stays skipped by its predecessor-completeness guard until the one-time `seed-charts` dispatch runs and the `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` secret is seeded.
- **Phase 3 (in PR):** the `deploy/` Kustomize base + Flux DAG (`feat/phase3-deploy-scaffold`, 28 bases, render/DAG/co-ownership verified) is reviewed and fixed. The config-artifact publish job and the `deploy-and-test` switch are the remaining sub-steps, now unblocked by Phase 2.
- **Phase 4 / 5 (not started):** site repos + SOPS + dev-cluster cutovers, then the on-prem cutover.
