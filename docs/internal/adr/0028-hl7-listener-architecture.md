...
- **Versions:** Strimzi 1.1.0 and Kafka 4.3.0 (Strimzi 1.1.0's supported default) are pinned in `group_vars/all/versions.yaml`. The listener app is Camel 4.22.0 on Spring Boot 4.1.x, pinned in `hl7-listener/build.gradle` (Dependabot watches it). Camel ships two LTS lines a year, each supported for a year — stay on one; 4.22 is supported to Aug 2027, the previous line (4.18) to Feb 2027. Each Camel minor targets one Spring Boot line, so a Camel bump past the current line is also a Spring Boot upgrade. Diff the component `*Constants` values between jars when bumping — Camel has renamed header strings in a patch release without documenting it.
