...
- **Versions:** Strimzi and Kafka are pinned in `group_vars/all/versions.yaml`; the listener app's Camel and Spring Boot versions in `hl7-listener/build.gradle`. Both are watched for upgrades (Renovate, Dependabot). Two constraints when bumping Camel: each Camel minor targets one Spring Boot line, so moving past the current line is also a Spring Boot upgrade, and Camel renames header constants — diff the component `*Constants` values between the old and new jars, since it has done this in a patch release without documenting it.
...
- **Batching:** a batch closes at the record cap, after the poll timeout of inactivity, or at the batching interval — whichever comes first. At production rates the record cap dominates; the poll timeout covers lulls; the interval bounds how long a batch can stay open when a feed is too slow to fill the cap but too steady to ever idle, which otherwise leaves a batch open for hours and makes the batcher look stalled. All three are tunable per environment (`hl7_batcher_max_poll_records` / `hl7_batcher_poll_timeout_ms` / `hl7_batcher_batching_interval_ms`). Offsets are committed only after each batch's durable write (at-least-once; see "Why batching").
