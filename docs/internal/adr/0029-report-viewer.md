...
In aws mode (ADR 0035) there is no oauth2-proxy or Traefik, so the browser path has neither the header nor the secret. The ALB authenticates the browser against Keycloak itself, reusing the `oauth2-proxy` client, and forwards the user's access token in `X-Amzn-Oidc-Accesstoken`. Setting `REPORT_VIEWER_FORWARDED_TOKEN_HEADER` to that header makes report-viewer validate the token exactly like a bearer, which is why the `oauth2-proxy` client also carries the `report-viewer-audience` scope. It is unset by default.
