...
**Rejected**: the bundle pattern is OPA's canonical mechanism for user-data distribution — its built-in plugin ships with atomic data-tree swap, ETag-conditional GETs, last-known-good on-disk persistence (`persist: true`), and readiness gating on first successful load. Going with `http.send` instead means reimplementing those behaviors against `force_cache_duration_seconds`: per-key invalidation, cache-bust threading, retry/timeout handling — all written into Rego that should be doing access control, not transport. Two operational behaviors fall out for free from leaning on OPA's bundle plugin: (a) during a Keycloak maintenance window, OPA serves the last-pulled snapshot from on-disk cache so live sessions with valid JWTs keep working, whereas `http.send` cache misses fail and sessions get denied within their TTL; and (b) bundle pull is uniformly 5–10s per replica, whereas per-key TTL caching converges quickly only for recently-touched keys.
