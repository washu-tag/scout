...
| Identity model | Per-user AuthZ attributes are Keycloak User Profile entries stored as `Map<String, List<String>>`: `allowed_facilities` (multivalued, supports `*` wildcard) for row scoping; `mask_phi_fields` (multivalued, defaults to no masking; `["true"]` opts in) for column masking. The policy is otherwise attribute-driven — no group check inside row-filter / column-mask logic — except for a single gate (see Approval gate row). |
