...
- **Keycloak realm template** (~~`ansible/roles/keycloak/templates/scout-realm.json.j2`~~ `helm/keycloak-config-cli/files/scout-realm.json`) renders the three service-principal clients with `serviceAccountsEnabled: true`, the `trino-audience` scope attached, and `access.token.lifespan` from ~~the literal `14400`~~ the chart's `lifespans.trinoServiceToken`. Secrets live in inventory under `keycloak_<name>_svc_client_secret`.
