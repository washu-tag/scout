...
| Identity model | Existing `scout-admin` and `scout-user` Keycloak groups gate admin/user permissions in other Scout apps; OPA's Trino policy itself is **purely attribute-driven** (no group check). Per-user attributes are Keycloak User Profile entries stored as `Map<String, List<String>>`: `allowed_facilities` (multivalued, supports `*` wildcard) for row scoping; `mask_phi_fields` (multivalued, defaults to mask; `["false"]` opts out) for column masking. |
