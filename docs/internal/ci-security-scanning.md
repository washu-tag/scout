...
**Mirrors in `deploy/`:** the GitOps bases keep third-party versions as literals, so a pin that also appears under `deploy/` needs the same `# renovate:` comment directly above its `version:`, `tag:` or `image:` line. Renovate groups updates by `depName`, so `versions.yaml` and its `deploy/` copies bump in one PR.
