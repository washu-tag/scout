...
**Keeping copies in sync:** the `tooling/versions` check in CI fails when copies of a pinned version disagree, so a hand edit to one copy (as in #706) is caught at review. The Keycloak Operator and CRDs vendored into `deploy/` are generated by `tooling/deploy/vendor_keycloak_operator.py`: Renovate runs it on Keycloak bumps, and after changing `keycloak_version` by hand you run it yourself.
