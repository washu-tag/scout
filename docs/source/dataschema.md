...
| `updated`                           |                  | timestamp       | No       | Timestamp of the last content change to the report row in the Delta Lake. Re-ingesting a report whose extracted content is unchanged does not modify the row or this timestamp.                                            |
| `content_hash`                      | Derived          | string          | Yes      | SHA-256 over the JSON serialization of all extracted columns (everything except `updated` and `content_hash` itself). Used internally to skip merging unchanged reports on re-ingest. NULL for rows last written before this column existed; backfilled the next time the same `source_file` is re-ingested. Not propagated to downstream tables. |
