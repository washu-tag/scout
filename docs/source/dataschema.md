...
| `patient_ids`                     | PID-3            | array<struct> | Yes      | Structured representation of all patient identifiers. Patient ID columns are also created for each assigning authority (e.g., `epic_mrn`) or assigning facility. See {ref}`Patient IDs <patient_ids_ref>` for more detail. |
...
| `assistant_result_interpreter`    | OBR-33           | array<string> | Yes      | Array of names within assistant result interpreter field in format "FIRST LAST". Duplicates and empty names are filtered out.                                                                                              |
| `technician`                      | OBR-34           | array<string> | Yes      | Array of names within technician field in format "FIRST LAST". Duplicates and empty names are filtered out.                                                                                                                |
| `diagnoses`                       | DG1-3            | array<struct> | Yes      | Diagnosis codes for the report. See {ref}`Diagnoses <diagnoses_ref>` for more detail.                                                                                                                                      |
