...
Usage in Notebook:
    from {your_module} import create_dashboard
    create_dashboard()
"""
...
def create_dashboard(table_name="default.reports", date_range_days=360):
    ...
    with status_output:
        ...
        tat_count = df["order_to_report_hours"].notna().sum()
...
def _render_summary(df):
    ...
    # TAT statistics
    mean_tat = df["order_to_report_hours"].mean()
    median_tat = df["order_to_report_hours"].median()
    p90_tat = (
        df["order_to_report_hours"].quantile(0.9)
        if df["order_to_report_hours"].notna().sum() > 0
        else None
    )
...
def _render_tat_by_modality(df):
    ...
    tat_by_mod = (
        df[df["order_to_report_hours"].notna()]
        .groupby("modality")
        .agg(
            count=("order_to_report_hours", "size"),
            median_hours=("order_to_report_hours", "median"),
            mean_hours=("order_to_report_hours", "mean"),
            p90_hours=("order_to_report_hours", lambda x: x.quantile(0.9)),
        )
        .reset_index()
    )
...
def _render_tat_trends(df):
    ...
    df_with_tat = df[df["order_to_report_hours"].notna()].copy()
...
def _render_radiologist_metrics(df):
    ...
    rad_metrics = (
        ...
        .agg(
            ...
            median_tat=("order_to_report_hours", "median"),
            mean_tat=("order_to_report_hours", "mean"),
