...
- **ADR 0014: Temporary Open WebUI Context Summarization Filter** — Installs a filter to handle long conversations that approach the 128K context window limit, preventing Ollama from silently truncating older messages. Consult when modifying Open WebUI filter functions or conversation handling.
...
- **ADR 0025: In-App User Administration and Launchpad Token Pass-Through** — The launchpad user console (`/admin/users`) is backed by a Keycloak SPI REST resource (`/realms/<realm>/scout-users`, in `keycloak/event-listener/`) that acts on the caller's own bearer (no standing admin credential), requires `aud=scout-users-api`, and discovers the editable data-access attributes dynamically from the User Profile (`scoutAuthz=true` annotations, rendered from `trino_attribute_filters`). Every SPI mutation explicitly emits a Keycloak admin event so OPA republish, approval/offboard emails, and the audit log work. The launchpad proxies via `/api/users/[...path]` (per-method path allowlist; mints a fresh access token from the session's refresh token per request). Consult when modifying the user console, the scout-users SPI, approval emails, or launchpad session/token handling.

- **ADR 0026: Delegated Capability Roles and the User Manager** — Capabilities are Keycloak realm roles (first: `manage-users`) assigned to groups in the realm template: `scout-user-manager` (new, additive over scout-user, maps nothing else) and `scout-admin` both map it, so every check — SPI `requireCapability`, launchpad session `roles` array — tests a single role name with no "or admin" logic and no boolean-per-role growth. Server-side invariants: promote/demote `scout-admin` stays admin-only; all capability-tier mutations reject scout-admin targets from non-admin callers (`requireCanModifyTarget`); manager grant requires the target already be in scout-user; offboard also removes the manager group. A user manager is not a realm admin, not an admin of other services, and OPA/rego is untouched. Accepted blast radius: a manager can self-grant full unmasked data access (audited). Consult when adding delegated roles, touching the SPI's authorization tiers, the launchpad role gating (`lib/roles.ts`), or the approval-email listener's group-event matching.
