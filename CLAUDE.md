...
- **ADR 0020: Trino RBAC via OPA with Keycloak Attributes** — Enforces per-user row filtering and column masking on `trino-analytics` via OPA, using Keycloak user attributes (`allowed_facilities`, `allowed_modalities`, `mask_phi_fields`). Adding a restriction dimension is one inventory edit (the keycloak role renders user-profile attributes from `trino_attribute_filters`; the opa role consumes the same map). Views ship as Trino's default `SECURITY DEFINER` with a `view_owner_principals` exemption so the view owner can materialize unrestricted reads; the invoker's RBAC is enforced at the view-output layer (views are listed in `trino_filtered_tables`). `view_only_tables` denies direct SELECTs on join-target tables like `reports_report_patient_mapping`. MCP impersonation shipped (`openwebui_mcp_svc`, OIDC inbound + `X-Trino-User` outbound); Voila adopts the same pattern with a separate `voila_svc` principal, implementation pending. Consult when modifying the OPA policy (`policy/trino/main.rego`), adding RBAC dimensions, working with the `_epic_view` views, or touching identity propagation in client roles (jupyter, superset, mcp-trino, voila).

- **ADR 0021: OPA User Attribute Distribution via MinIO Bundles** — Supersedes the cache-bust follow-up to ADR 0020. The Keycloak SPI listener (`OpaUserBundlePublisherProvider` in `keycloak/event-listener/`) maintains an in-memory user-attribute snapshot, debounces admin events into S3 PUTs of a tar.gz bundle to a dedicated MinIO bucket (`opa-bundles`), and OPA's native bundle plugin pulls every 5–10s and atomically swaps the `data.users` subtree. The Rego policy reads `data.users[user]` directly — no `http.send`, no admin-token client. Consult when modifying the bundle publisher, OPA's bundle plugin config, or the rego's user-attribute lookup path.
