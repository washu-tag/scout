...
- **ADR 0033: Build-Lane Bundling and Air-Gap Transport via Hauler** (Proposed) — Refines ADR 0030 §1 / 0031 §7. The build manifest is realized as a signed **Hauler** haul: `hauler store sync -f <manifest> --key` + `store save` produce one `.tar.zst` carrying every image and OCI chart pinned by digest with its keyed cosign signature; the air-gap mirror step is `hauler store copy registry://harbor` (digests preserved byte-for-byte, POC-validated), and Flux stays the consumer unchanged (`OCIRepository`/`chartRef` + `.spec.verify`). Retires the custom JSON build manifest + schema/validator, the oras/mirror publish job, and `classify.py`'s scope enum; residual custom shrinks to a thin "resolve digests → render Hauler manifest" step. Keyed cosign only (no keyless/Sigstore tlog across the gap). Open sub-decision: per-artifact signatures (recommended) vs one whole-platform signature (imgpkg/Zarf stronger there). Consult when touching build-lane bundling, signing, or air-gap transport.
