...
- **ADR 0020: Trino RBAC via OPA with Keycloak Attributes** — Enforces per-user row filtering and column masking on `trino-analytics` via OPA, using Keycloak user attributes (`allowed_facilities`, `allowed_modalities`, `mask_phi_fields`) fetched at policy-evaluation time. Adding a restriction dimension is one inventory edit (the keycloak role renders user-profile attributes from `trino_attribute_filters`; the opa role consumes the same map). Views ship as Trino's default `SECURITY DEFINER` with a `view_owner_principals` exemption so the view owner can materialize unrestricted reads; the invoker's RBAC is enforced at the view-output layer (views are listed in `trino_filtered_tables`). `view_only_tables` denies direct SELECTs on join-target tables like `reports_report_patient_mapping`. Real-time invalidation via Keycloak event listener and MCP/Voila wiring are deferred. Consult when modifying the OPA policy (`policy/trino/main.rego`), adding RBAC dimensions, working with the `_epic_view` views, or touching identity propagation in client roles (jupyter, superset).
