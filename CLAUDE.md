...
- **ADR 0024: Token Refresh for SDK Trino Access** — Keeps the short-lived bearer (ADR 0022) valid for notebook/playbook code without OAuth in user code, via four layers: (1) custodian-side proactive rotation — Voila re-mints `voila_svc`; Jupyter relies on an oauthenticator `refresh_user_hook` (`eagerTokenRefresh`) that rotates at token half-life because stock oauthenticator only re-validates a live token without rotating; (2) SDK proactive re-fetch within 60s of expiry via a per-request dynamic bearer (one long-lived SQLAlchemy engine survives rotation, no `engine.dispose()`); (3) SDK reactive — `query()` retries once on Trino 401/403, dropping the cached bearer first (`connect()` doesn't wrap caller-driven execution); (4) per-kernel `threading.Lock` single-flight. `auth_refresh_age = keycloak_access_token_lifespan // 5` (gate must reopen before the SDK's 60s window); `keycloak_access_token_lifespan` drives both the realm `accessTokenLifespan` and the Hub gate so they can't drift. Consult when touching the SDK's `_query.py`/`_identity.py` refresh logic, the Jupyter `eagerTokenRefresh` hook / `auth_refresh_age`, or debugging Trino `JWT expired`/401 from notebooks.
