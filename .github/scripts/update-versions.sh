...
update_file "ansible/roles/report_viewer/defaults/main.yaml" \
...
update_file "report-viewer/pyproject.toml" \
    '^(version = ")[^"]+(")'  \
    "\\1$PYTHON_VERSION\\2" \
    "pyproject.toml version"
...
for chart in helm/launchpad/Chart.yaml \
             ...
             helm/keycloak-fragment-reconciler/Chart.yaml \
...
update_file "helm/keycloak-fragment-reconciler/values.yaml" \
    "^(  tag:) .+$" \
    "\\1 $DOCKER_TAG" \
    "keycloak-fragment-reconciler image.tag"
