...
(the cohort-building POC). Lives in the service so the OWUI tool stays
thin and the same logic is reusable by other Scout surfaces (notebooks,
batch jobs, future patient-cohort tooling).
...
table + the internal cohort-handle note.
"""
...
# --------------------------------------------------------------------------
# Pattern banks. Mirror analytics/notebooks/cohort/cohort_builder.py —
...
def build_search_summary(
    *,
    ...
    if negation_applied:
        ...
        parts.append(
            ...
            + (
                ""
                if saved_cohort_id
                else " (Aggregate result — no cohort saved. Include "
    ...
    iframe_will_render = bool(sample_rows and saved_cohort_id)
    ...
    if sample_rows:
        ...
        if total_rows > len(sample_rows):
            parts.append(
                ...
                f"NOT in this sample — they're in the saved cohort and "
                f"in the user's interactive table. Reason about the cohort "
    ...
    if saved_cohort_id:
        parts.append("")
        parts.append(
            f"Internal cohort handle for chaining: {saved_cohort_id}. "
