"""HTTP routes for `/datasets` (V1.1 — just-in-time SQL evaluation).
...
Endpoints:
  POST /datasets              — save SQL, cache COUNT(*), return sample
  POST /datasets/from-ids     — validate IDs against reports_latest, save WHERE id IN (...) SQL
  POST /datasets/aggregate    — ad-hoc SQL, return rows directly (scout_query_sql backend)
  GET  /datasets/{id}         — metadata
  GET  /datasets/{id}/rows    — paginated rows (wraps source_sql)
  GET  /datasets/{id}/reports/{report_id} — single full report from a cohort
  GET  /datasets/{id}/accessions — DISTINCT accession_number list
  GET  /datasets/{id}/export.csv — streaming CSV
"""
...
from ..ids import new_dataset_id
from ..models import (
    KNOWN_ID_COLUMNS,
    AggregateRequest,
    AggregateResponse,
    ...
    CreateFromIdsRequest,
    CreateFromIdsResponse,
    DatasetMeta,
    RowsResponse,
)
...
# Safety cap on submitted IDs for /datasets/from-ids. The same cap
# becomes the upper bound on the IN-clause length in the saved SQL.
_MAX_FROM_IDS = 1_000_000
...
def _view_url(request: Request, dataset_id: str) -> str:
    # The OWUI tool overwrites scheme+host via its public_base_url
    # valve to point at the chat-host same-origin alias; we just
    # supply the path. `/spa/datasets/{id}` is the React detail page.
    base = str(request.base_url).rstrip("/")
    return f"{base}/spa/datasets/{dataset_id}"
...
# ---------------------------------------------------------------------------
# POST /datasets/aggregate — scout_query_sql backend, no persistence
# ---------------------------------------------------------------------------
...
async def aggregate(
    ...
    """Run SQL once, return rows directly. No cohort persisted, no
    iframe rendered. Backs `scout_query_sql` in the OWUI tool."""
    try:
        with metrics.time_trino("aggregate_query"):
            columns, rows = await trino_client.execute(
                body.sql, user=user.sub, token=user.token
            )
    except Exception as exc:
        log.exception("trino aggregate query failed")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    rows = _jsonsafe(rows)
    return AggregateResponse(columns=columns, rows=rows, truncated=truncated)
...
async def create_dataset(
    ...
    """Save a SQL query as a cohort. No row materialization — runs one
    ...
    Refinement: when the LLM wants to narrow a cohort, it writes a new
    `POST /datasets` call with the original conditions plus the new
    constraint — the saved SQL is standalone, no placeholder
    substitution, no parent reference required at SQL time.
    `parent_id` on the dataset row is informational lineage only.
    """
    source_sql = _wrap_source_sql(body.sql)
    parent_id = body.parent_dataset_id  # lineage only — not used in SQL

    # Validate the cohort SQL by fetching the first 5 sample rows.
    ...
    try:
        ...
        metrics.DATASETS_CREATED.labels(
    ...
    try:
        ...
        # Non-fatal: cohort still gets created, just without cached
    ...
    stored = await store.insert_dataset(
    ...
    metrics.COHORT_SIZE.labels(kind=body.kind, source="sql").observe(stored["count"])
    log.info(
        "dataset created",
        extra={
    ...
    summary = _build_summary(
        ...
        columns=columns,
        sample_rows=sample,
        saved_cohort_id=stored["dataset_id"],
    )
    ...
    return CreateDatasetResponse(
        ...
        kind=stored["kind"],
        sample=sample,
        view_url=_view_url(request, dataset_id),
        summary=summary,
    )


# ---------------------------------------------------------------------------
# POST /datasets/from-ids — validate IDs, save WHERE id IN (...) SQL
# ---------------------------------------------------------------------------


@router.post(
    "/from-ids",
    response_model=CreateFromIdsResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_dataset_from_ids(
    body: CreateFromIdsRequest,
    request: Request,
    user: User = Depends(get_current_user),
) -> CreateFromIdsResponse:
    """Materialize a cohort from a researcher-supplied ID list. The
    ...
    `WHERE <id_col> IN ('a', 'b', ...)` SQL as the cohort's source.

    Same downstream shape as POST /datasets — the saved SQL is what
    ...
    if len(cleaned) > _MAX_FROM_IDS:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=(
                f"submitted ID list exceeds cap ({_MAX_FROM_IDS}); "
    ...
    for start in range(0, len(cleaned), CHUNK):
        ...
        try:
            with metrics.time_trino("from_ids_validate"):
    ...
    # Compose the saved cohort SQL. The same id_column is selected
    ...
    metrics.COHORT_SIZE.labels(kind=body.kind, source="from_ids").observe(
        stored["count"]
    )
    ...
    if unmatched:
        summary_lines.append(
            ...
            f"were dropped from the cohort."
        )
    summary_lines.append("")
    summary_lines.append(
        "USER DISPLAY: the matched IDs render as a cohort in the viewer "
    ...
    summary_lines.append("")
    summary_lines.append(
        f"Internal cohort handle for chaining: {stored['dataset_id']}."
    )

    return CreateFromIdsResponse(
        dataset_id=stored["dataset_id"],
        count=stored["count"],
...
async def get_dataset_meta(
    ...
    ds = await store.get_dataset(dataset_id, user.sub)
    if ds is None:
...
# ---------------------------------------------------------------------------
# GET /datasets/{id}/rows — wrap source_sql + paginate / sort / filter
# ---------------------------------------------------------------------------
...
async def get_dataset_rows(
    ...
    """Paginated dataset rows. Wraps the saved source_sql as a
    ...
    ds = await store.get_dataset(dataset_id, owner_sub=None)
    if ds is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    ...
    if not source_sql:
        raise HTTPException(
            ...
            detail="this cohort has no source SQL (pre-V1.1 dataset)",
        )
...
async def get_dataset_report(
    ...
    """Fetch the full row for one report in a cohort. Membership check
    ...
    if not rows:
        ...
        raise HTTPException(
            ...
            detail=f"report {report_id!r} not in dataset {dataset_id}",
        )
...
# ---------------------------------------------------------------------------
# GET /datasets/{id}/accessions
# ---------------------------------------------------------------------------


@router.get("/{dataset_id}/accessions")
async def get_dataset_accessions(
    dataset_id: str,
    user: User = Depends(get_current_user),
) -> dict[str, Any]:
    ds = await store.get_dataset(dataset_id, owner_sub=None)
...
# ---------------------------------------------------------------------------
# GET /datasets/{id}/export.csv — streaming, page-through via OFFSET/LIMIT
# ---------------------------------------------------------------------------
...
def _build_summary(
    *,
    ...
    """LLM-bound markdown summary returned alongside the dataset."""
    parts: list[str] = []
    ...
    parts.append("")
    parts.append(
        f"Internal cohort handle: {saved_cohort_id}. Keep this backstage; "
        f"only mention it to the user when discussing the cohort by name. "
