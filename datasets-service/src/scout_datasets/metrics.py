...
the cohort-specific counters that the LLM-bound and viewer paths care
...
gated on `enable_datasets_service` in inventory.
"""
...
#   - `result` ∈ {"ok", "error"} (~2)
# Anything user-derived (owner_sub, dataset_id) stays OUT of labels.

DATASETS_CREATED = Counter(
    "scout_datasets_created_total",
    "Datasets saved via POST /datasets.",
    ["kind", "id_column", "result"],
)

TRINO_QUERY_DURATION = Histogram(
    "scout_datasets_trino_query_duration_seconds",
    "Time spent in Trino per call from the datasets service.",
    ["op", "result"],
    buckets=(0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0, 120.0),
)

POSTGRES_OP_DURATION = Histogram(
    "scout_datasets_postgres_op_duration_seconds",
    "Time spent in Postgres per dataset CRUD op.",
    ["op", "result"],
    buckets=(0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0),
)

# Cohort size distribution — wide log-ish buckets so we can see "most
# cohorts are ~hundreds, a few are big". Recorded once per successful
# create_dataset / create_from_ids.
COHORT_SIZE = Histogram(
    "scout_datasets_cohort_size_rows",
    "Row count of each cohort at create time (cached COUNT(*) of the saved SQL).",
    ["kind", "source"],  # source ∈ {"sql", "from_ids"}
...
IDS_SUBMITTED = Counter(
    "scout_datasets_ids_submitted_total",
    "Total identifiers submitted via /datasets/from-ids.",
    ["id_column"],
)

IDS_UNMATCHED = Counter(
    "scout_datasets_ids_unmatched_total",
...
OWUI_WEBHOOK_EVENTS = Counter(
    "scout_datasets_owui_webhook_events_total",
...
REPORT_FETCH = Counter(
    "scout_datasets_report_fetch_total",
...
def install(app: FastAPI) -> None:
    ...
    instrumentator = (
        Instrumentator(
            ...
            inprogress_name="scout_datasets_http_requests_in_progress",
