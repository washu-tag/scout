...
1. **Bearer JWT** validated against Keycloak JWKS. Used by the OWUI tool
   ...
   `datasets-service` Keycloak client yet — flip `aud_verify=True` in
...
3. **Dev shared secret** (`X-Datasets-Shared-Secret` + `X-Datasets-Test-User`)
...
class User:
    sub: str  # owner_sub stored on the dataset row
...
async def get_current_user(
    ...
    x_datasets_test_user: str | None = Header(default=None),
    x_datasets_shared_secret: str | None = Header(default=None),
) -> User:
    ...
    if (
        ...
        and x_datasets_shared_secret == settings.dev_shared_secret
        and x_datasets_test_user
    ):
        return User(sub=x_datasets_test_user)
