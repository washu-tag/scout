...
def test_view_returns_html_for_owner(client, auth_headers, fake_trino):
    ...
    r = client.get(f"/datasets/{dsid}/view", headers=auth_headers)
    assert r.status_code == 200
    assert "text/html" in r.headers["content-type"]
    # Page contains the dataset id and the Tabulator integrity tag —
...
def test_accessions_returns_deduped_list(client, auth_headers, fake_trino):
    ...
    r = client.get(f"/datasets/{dsid}/accessions", headers=auth_headers)
    assert r.status_code == 200
...
def test_export_csv_streams_with_header_and_rows(client, auth_headers, fake_trino):
    ...
    r = client.get(f"/datasets/{dsid}/export.csv", headers=auth_headers)
    assert r.status_code == 200
...
def test_export_csv_empty_dataset_returns_header_only(client, auth_headers, fake_trino):
    # We can't actually have an empty dataset (POST rejects), but a
