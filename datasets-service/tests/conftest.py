...
Postgres-bound tests look for `DATASETS_TEST_DATABASE_URL` and are
...
from scout_datasets import db, trino_client
from scout_datasets.app import create_app
from scout_datasets.config import settings


TEST_DB_URL = os.environ.get("DATASETS_TEST_DATABASE_URL", "")


def _needs_pg(request):
    if not TEST_DB_URL:
        pytest.skip(
            "DATASETS_TEST_DATABASE_URL not set — Postgres-backed tests skipped"
        )


@pytest_asyncio.fixture
async def reset_schema():
    """Drop and recreate the `datasets` table so each test starts clean."""
    _needs_pg(None)
    settings.database_url = TEST_DB_URL
    await db.close_pool()
    async with db.get_conn() as conn:
        async with conn.cursor() as cur:
            await cur.execute("DROP TABLE IF EXISTS datasets CASCADE")
        await conn.commit()
...
def fake_trino(monkeypatch) -> Callable[[list[str], list[dict[str, Any]]], None]:
    ...
    `create_dataset` -> `get_rows` -> `get_summary` can prime distinct
