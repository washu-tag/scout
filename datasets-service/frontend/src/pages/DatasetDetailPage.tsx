...
import { getDataset, getDatasetRows, getReport } from '../api/client';
...
export default function DatasetDetailPage() {
  ...
  const rowsQ = useQuery({
    queryKey: ['dataset', datasetId, 'rows', page, limit, sortParam, filters],
    queryFn: () => getDatasetRows(datasetId, { page, limit, sort: sortParam, filters }),
    enabled: !!datasetId,
  ...
  // the post-filter count). Fall back to dataset meta when there's no
  // filter applied.
          ...
          <>
            ...
            >
              ...
              {/* "Apply filter to chat" — only meaningful when filters are
                ...
                rebuilds the cohort with the same filters baked into the
                SQL. */}
              ...
              <a
                href={`/datasets/${encodeURIComponent(datasetId)}/export.csv`}
                style={{
                  ...(embedded ? paginationBtnEmbed : paginationBtn),
                  color: '#222',
                  textDecoration: 'none',
...
function applyFilterToChat(filters: Record<string, string>) {
  ...
  // via parent_dataset_id when the user asks to filter an existing
  // cohort, so we don't need to spell out "rebuild" / "refine the
  // saved cohort" / etc. — just say what to filter on.
...
function SendToXnatModal(props: { datasetId: string; total: number; onClose: () => void }) {
  ...
  const onSend = () => {
    alert(
      `TBD — XNAT push not wired yet.\n\nWould send ${props.total} accessions from ${props.datasetId} to project "${project}" under IRB ${irb}.`,
    );
    props.onClose();
  };
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ color: '#555', margin: '0 0 1rem', fontSize: '0.85rem' }}>
          ...
          <code>{props.datasetId}</code> to an XNAT project. Walk through each step before sending.
        </p>
...
function RowDetail(props: {
  ...
  // The row's identifier for the /reports lookup. The dataset's
  // id_column value lives on the slim /rows row under the same key.
  const reportId = String(props.row[props.idColumn] ?? '');
  const reportQ = useQuery({
    queryKey: ['report', props.datasetId, reportId],
    queryFn: () => getReport(props.datasetId, reportId),
    enabled: !!reportId,
  ...
  // Identify which diagnosis codes / texts match the cohort's
  ...
  return (
    <div style={{ fontSize: '0.78rem', lineHeight: 1.4, color: '#222' }}>
      ...
      {/* Diagnoses chips — TOP of panel so the user sees the ICD
          ...
          cohort's highlight terms) get a yellow background + bold so
