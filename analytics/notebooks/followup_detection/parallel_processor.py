...
def process_reports_in_batches(
    ...
    resp = requests.post(
        f"{ollama_url}/api/generate", json={"model": model, "keep_alive": -1}
    )
    ...
    df_to_process = spark.table(dest_table).filter(
        F.col("followup_processed_at").isNull()
    )
    ...
    if priority_filter:
        ...
        print(
            f"  Priority reports (with keywords): {priority_reports:,} ({100*priority_reports/total_reports:.1f}%)"
        )
        print(
            f"  Non-priority reports: {non_priority_reports:,} ({100*non_priority_reports/total_reports:.1f}%)"
        )
    ...
    for batch_num in tqdm(range(num_batches), desc="Batches"):
        ...
        df_query = spark.table(dest_table).filter(
            F.col("followup_processed_at").isNull()
        )
        ...
        if priority_filter:
            ...
            if len(df_batch_pandas) == 0:
                print(
                    "  ✅ All priority reports processed, moving to remaining reports..."
                )
                df_batch_pandas = (
                    df_query.limit(batch_size)
                    .orderBy(F.col("message_dt").desc())
                    .select("message_control_id", "report_text")
                    .toPandas()
                )
        ...
        with ThreadPoolExecutor(max_workers=total_workers) as executor:
            ...
            with tqdm(
                total=len(df_batch_pandas),
                desc=f"  Batch {batch_num+1} reports",
                leave=False,
            ) as pbar:
    ...
    print(
        f"  Overall throughput: {(total_processed+total_errors)/overall_elapsed:.2f} reports/sec"
    )
