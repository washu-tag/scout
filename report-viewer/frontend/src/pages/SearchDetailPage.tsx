...
export default function SearchDetailPage() {
  ...
  const showLoading = useDelayedLoading(!rowsQ.data && rowsQ.isLoading);
  const fetchProgress = useCallback(() => getSearchProgress(searchId), [searchId]);
  const rowsProgress = useQueryProgress(showLoading, fetchProgress);
  ...
  return (
    ...
    >
      ...
      >
        {showLoading && <QueryProgressInline progress={rowsProgress} />}
        {rowsQ.data && (
      ...
      {!rowsQ.data && rowsQ.isLoading ? (
        <LoadingSpinner show={showLoading} minHeight={HEIGHT_COMPACT / 2} />
      ) : (
        rowsQ.data && (
          <div
            style={{
