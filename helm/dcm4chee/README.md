...
`dcm4chee_admin_user`: WildFly ManagementRealm username seeded on first boot for the admin console. Defaults to `admin`.
`dcm4chee_admin_password`: Password for that user. **Required when `dcm4chee_expose_management_console` is `true`** (the deploy asserts it) — vault-encrypt it. Seeded only on first boot; the account persists on the wildfly PVC, so changing this later does not rotate it (re-run `add-user.sh` in the pod or recreate the PVC).
