"""
title: Dataset iframe lift
description: Outlet filter — appends an HTML artifact code block (renders in OWUI's side artifact panel with allow-scripts sandbox) for the last Scout dataset_id mentioned. Replaces the raw markdown <iframe> approach, which OWUI rewrites to sandbox="" and blocks scripts inside.
"""
...
_DATASET_ID_RE = re.compile(r"\b(ds_[A-Za-z0-9]{6})\b")
_MARKER_BEGIN = "<!-- BEGIN scout-dataset-artifact -->"
_MARKER_END = "<!-- END scout-dataset-artifact -->"
...
def lift_iframe(content: str, base_url: str) -> str:
    ...
    `html` code block for the LAST dataset_id mentioned. OWUI auto-detects
    ...
    dataset_id is present and there's nothing to strip."""
    stripped = _INJECTED_BLOCK_RE.sub("", content)
    matches = _DATASET_ID_RE.findall(stripped)
    if not matches:
        return stripped
    dataset_id = matches[-1]
    base = base_url.rstrip("/")
    src = f"{base}/datasets/{dataset_id}/view"
    ...
    # browsing context.
    # Two-part block:
    #   1. A `[View dataset ↗](url)` markdown link — always works as a
    ...
    block = (
        f"\n\n{_MARKER_BEGIN}\n"
        f"[View dataset {dataset_id} ↗]({src})\n\n"
        f"```html\n"
        f"<!DOCTYPE html>\n"
        f'<html lang="en">\n'
        f'<head><meta charset="utf-8"><title>Dataset {dataset_id}</title>\n'
...
class Filter:
    class Valves(BaseModel):
        ...
        base_url: str = Field(
            default="https://datasets.dev02.tag.rcif.io",
            description="Public origin where the datasets-service viewer is reachable.",
        )
    ...
    async def outlet(
        ...
        print("[dataset_iframe_lift] outlet called", flush=True)
        messages = body.get("messages") or []
        for msg in messages:
            if msg.get("role") != "assistant":
                continue
            original = msg.get("content") or ""
            # Cheap pre-check — skip messages with no dataset id.
            if "ds_" not in original:
                continue
