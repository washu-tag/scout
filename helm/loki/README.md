...
Grafana Alloy is used to collect logs from the Kubernetes cluster and send them to Loki. See `ansible/roles/alloy/` for the deployed configuration. To install standalone for development:

```bash
helm upgrade --install alloy grafana/alloy --namespace loki --version 1.5.1
```
