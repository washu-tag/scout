...
{{/*
Render the configmap items list — one entry per analytics file, mapping the
configmap key (file basename) to the mount path Superset's import-dashboards
command expects (analytics/<subdir>/<file>).
*/}}
{{- define "scout-dashboards.configMapItems" -}}
{{- range $path, $_ := .Files.Glob "files/analytics/charts/*.yaml" }}
- key: {{ base $path }}
  path: analytics/charts/{{ base $path }}
{{- end }}
{{- range $path, $_ := .Files.Glob "files/analytics/dashboards/*.yaml" }}
- key: {{ base $path }}
  path: analytics/dashboards/{{ base $path }}
{{- end }}
{{- range $path, $_ := .Files.Glob "files/analytics/datasets/Scout_Data_Lake/*.yaml" }}
- key: {{ base $path }}
