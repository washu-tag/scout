"""The reconciler's status: its vocabulary, its shape, and where it is kept.
...
`service` fills a `State` in and everything else reads one, which is why the
type lives here with the document it is rendered to rather than in `service`.
"""
...
# What became of a discovered fragment.
...
# What became of the realm. `Holding` and `Refused` both leave it alone: a
# retraction waiting out its grace period, and discovery never having reported
# a sync.
